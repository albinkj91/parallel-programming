 ...
 *
 *           The program had a *very serious bug* which we fixed during the lab
 *
...
/* Naive implementation, without synchronization.
   Will give incorrect results
 */
void* naive_thread_sum(void *rank) {
...
/* Uses a flag for synchronization between threads
   Will yield slow performance
 */
void* slightly_less_naive_thread_sum(void *rank) {
  long my_rank = (long) rank;
  double factor;
  long long i;
  long long my_n = (n / thread_count);
...
int main(int argc, char* argv[]) {
  ...
  printf("   Our estimate of pi = %.15f in %e seconds\n", sum, elapsed);

  GET_TIME(start);
  sum = Serial_pi(n);
  GET_TIME(end);
  elapsed = end - start;
  printf("   Single thread est  = %.15f in %e seconds\n", sum, elapsed);
  printf("                   pi = %.15f\n", 4.0*atan(1.0));
