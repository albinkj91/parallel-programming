...
void main_process(int argc, char** argv, int numOfProcesses) {
    ...
    int* start_end_numOfPrimes = new int[3] {start, max + 1, static_cast<int>(primes.size())};
    MPI_Bcast(start_end_numOfPrimes, 3, MPI_INT, 0, MPI_COMM_WORLD);
...
void worker_process(int numOfWorkers, int rank) {
    ...
    MPI_Bcast(primes, start_end_numOfPrimes[2], MPI_INT, 0, MPI_COMM_WORLD);
    ...
    int start = start_end_numOfPrimes[0] + rank - 1;
    int end = start_end_numOfPrimes[1];
    int numOfPrimes = start_end_numOfPrimes[2];

    int sum = 0;
    bool cont = false;
    for(int i = start; i < end; i += numOfWorkers)
    {
        cont = false;
